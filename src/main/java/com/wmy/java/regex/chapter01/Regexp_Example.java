package com.wmy.java.regex.chapter01;
 ...
 * @package_name: com.wmy.java.regex.chapter01
 * @Author: wmy
 * @Date: 2021/10/4
 * @Major: 数据科学与大数据技术
 * @Post：大数据实时开发
...
public class Regexp_Example {
    ...
    public void test1() {
        ...
        // 1、先创建一个Pattern对象，可以理解为正则表达式对象
        Pattern pattern = Pattern.compile("([a-zA-Z]+) | (0-9)+");

        // 2、创建一个匹配器对象,找到就返回true，否则就返回false
        Matcher matcher = pattern.matcher(content);

        // 3、可以开始循环的匹配
        while (matcher.find()) {
            // 匹配的内容和文本会放到 m.group()对象中去
    ...
    public void test4() {
        ...
        Matcher matcher = pattern.matcher(content);// 按照正则表达式的规则匹配字符串

        // 开始匹配
        /**
         * mather.find()
         * 1、根据指定的规则来定位，满足规则的子字符串,如果找到了1998，找到后，将这个位置的子字符串的开始的索引记录在matcher属性中
         * 2、public String group(int group);
         * group(0) = 0 把该字符串结束的索引加1，添加到group(1)=4;
         * 3、同时记录oldLast 的值就为即是，即下一次执行find()从4开始匹配
         * 4、group(0)：根据 0  和 1 的位置截取content截取这个子串，从0-4，包含0，4的位置，右端是不包含的
         * 如果再一次执行find() 仍然按照这个规则执行，这样的话就行清空的处理，会重新的进行输出操作
         */
        while (matcher.find()) {
